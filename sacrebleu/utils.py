...
def print_results_table(results: dict, signatures: dict, args: Namespace):
    ...
    if args.paired:
        test_type = 'bootstrap resampling' if args.paired == 'bs' \
            else 'approximate randomization'
        test_sample_type = 'resampling trials' if args.paired == 'bs' else 'trials'
        msg = f'Paired {test_type} test with {args.paired_n} {test_sample_type}'
        ...
        if args.paired == 'bs' or args.paired_ar_confidence_n != -1:
            print('   Actual system score / estimated true mean / 95% CI are provided for each metric.')
        else:
        ...
        print('   generated by the same underlying process. The p-value is roughly the probability')
        print('   of the absolute score difference (delta) between a system and the {bline} occurring due to chance.')
        print()
